...
// realiza pagamento em um Despesas
const efetuaPagamento = async (req, res) => {
  try {
    validateFields(req.body);

    const property = await Services.efetuaPagamento(req.body);
    res.status(200).send(handleSuccess(property));
  } catch (error) {
    console.log(error);
    res.status(400).send(handleError(error));
...
export default { findAll, getByPk, efetuaPagamento, count, add, update, destroy };
